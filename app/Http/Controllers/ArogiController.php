...
class ArogiController extends Controller
{
    ...
    public function foregion()
    {
        ...
        if($usertype=='2')
        {
            ...
            foreach($fos as $fo)
            {
                $html.= "<option value='".$fo->rob_fo."'>".$fo->rob_fo."</option>";
            }
            echo $html;
        }
        else //add 1 apr
    ...
    public function post_form_showpdf($id='')  //temp hide
    {
        ...
        if(Session::get('UserID'))
        {
            if(!empty($id))
            {
                ...
                return $pdf->download('ATP Receipt.pdf');
    ...
    public function pre_active_form($id='')
    {
        if(Session::get('UserID'))
        {
            if(!empty($id))
            {
                ...
                else if($usertype=='3' || $usertype=='5') //add 26 july
                {
                    $findFobData=DB::table('fob_details')->select('*')->where('FobName',$userName)->first();
                    $robid=$findFobData->RobId;
                    $robdetail=DB::table('rob_details')->select("*")->where("RobId",$findFobData->RobId)->first();
                ...
                // return $list;
                // return view('rob.dataform',["data"=>$data,"special_data"=>$list]);
                return view('rob.pre-activity-form',compact('data','offTyp','offRegion','ministry_data','demographys','area_data','download_display'));
            }
            else
            {
                $userName=Session::get('UserName');
                ...
                $download_display='none';
                return view('rob.pre-activity-form',compact('offTyp','offRegion','ministry_data','demographys','area_data','download_display'));
            }
        }
    }
    ...
    public function pre_form_showpdf($id='')
    {
        ...
        if(Session::get('UserID'))
        {
            if(!empty($id))
            {
                ...
                return $pdf->download('TTP Receipt.pdf');
            }
            else
            {
...
public function fob_TTP_form_showpdf($id='')
    ...
    public function approve_rejected($status,$id)
    {
        ...
        if($update_status)
        {
            if($status == 1)
            {
                ...
                return redirect()->back()->with("success","Status has been approved.");
            }
            else
            {
                ...
                Mail::send('rob.mail',$dat,function($msg) use($user){
                 ...
                 return redirect()->back()->with("success","Status has been rejected.");
            }
        }
        else
        {
    ...
    //pre list pdf download
     public function preroblistPDF($user_id)
     {
         ...
         return $pdf->download('TTP List.pdf');
     }
    ...
    //chekbox update rob and adg
     ...
     //all ro list
     //all-ro-list
     public function all_ro_list()
     {
        $data=DB::table('rob_details')->get();
        return view('rob.admin.all-ro-list',compact("data"));
     }

     //all fo list
     //all-fo-list
     public function all_fo_list($id)
     {
        $data=DB::table('fob_details')->where('RobId',$id)->get();
        return view('rob.admin.all-fo-list',compact("data"));
     }

     //all ro record
     //all_ro_record
     public function all_ro_record($robname)
     {
        $data=DB::table('rob_forms')->where('office',$robname)->get();
        return view('rob.admin.all-ro-record',compact("data"));
     }

     //all fo record
     //all-fo-record
     public function all_fo_record($fobname)
     {
        $data=DB::table('rob_forms')->where('office',$fobname)->get();
        return view('rob.admin.all-fo-record',compact("data"));
     }
